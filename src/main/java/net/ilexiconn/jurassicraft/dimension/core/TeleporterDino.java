...
public class TeleporterDino extends Teleporter
{
	...
	public TeleporterDino(WorldServer world)
	{
		super(world);
	...
	public void placeInPortal(Entity entity, double par2, double par4, double par6, float par8)
	{
		...
		{
			int spawnX = MathHelper.floor_double(entity.posX);
			int spawnY = MathHelper.floor_double(entity.posY) - 1;
			int spawnZ = MathHelper.floor_double(entity.posZ);
			...
			for (int l = -2; l <= 2; ++l)
			{
				for (int i1 = -2; i1 <= 2; ++i1)
				{
					...
					{
						int k1 = spawnX + i1 * b0 + l * b1;
						int l1 = spawnY + j1;
						int i2 = spawnZ + i1 * b1 - l * b0;
						boolean flag = j1 < 0;

						this.worldServerInstance.setBlock(k1, l1, i2, Blocks.stonebrick, 0, 0);
					}
				}
			}
			entity.setLocationAndAngles((double)spawnX, (double)spawnY, (double)spawnZ, entity.rotationYaw, 0.0F);
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8)
	{
		...
		Block portalBlock = ModBlocks.dinoPortal;
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
				{
					...
					for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2)
					{
						if (this.worldServerInstance.getBlock(k1, i2, l1) == portalBlock)
						{
							...
							spawnZ = (double)i2 + 0.5D - entity.posY;
							double d7 = d5 * d5 + spawnZ * spawnZ + d6 * d6;
							if (d3 < 0.0D || d7 < d3)
							{
								d3 = d7;
								i = k1;
		...
		if (d3 >= 0.0D)
		{
			...
			double spawnX = (double)i + 0.5D;
			double spawnY = (double)j + 0.5D;
			spawnZ = (double)k + 0.5D;
			int j2 = -1;
			...
			if (j2 > -1)
			{
				...
				spawnX += (double)((float)k3 * f1 + f2 * (float)i3);
				spawnZ += (double)((float)l3 * f1 + f2 * (float)j3);
				...
				entity.rotationYaw = par8 - (float)(teleportDirection * 90) + (float)(j2 * 90);
			}
			else
			{
				entity.motionX = entity.motionY = entity.motionZ = 0.0D;
		...
		for (i3 = 0; i3 < 4; ++i3)
		{
			for (j3 = 0; j3 < 4; ++j3)
			{
				for (k3 = -1; k3 < 4; ++k3)
				{
					...
					this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? Blocks.stonebrick : ModBlocks.dinoPortal), 0, 2);
				}
			}
