...
public class EntityJurassiCraftCreature extends EntityCreature implements IEntityAdditionalSpawnData, IAnimatedEntity
{
    ...
    public EntityJurassiCraftCreature(World world)
    {
        ...
        this.setCreatureGender(JurassiCraftDNAHandler.getDefaultGenderDNAQuality(this.getDNASequence()) == 0.5F ? this.rand.nextBoolean() : (JurassiCraftDNAHandler.getDefaultGenderDNAQuality(this
                .getDNASequence()) > 0.5F));
    ...
    public void showDebugInfo(EntityPlayer player)
    {
        ...
        if (this.worldObj.isRemote)
        {
            ...
            System.out.println("Genetic Quality: " + this.getGeneticQuality() + ", DNASequence: " + this.getDNASequence() + ". Revised DNA for 50% "
                    + JurassiCraftDNAHandler.reviseDNA(this.getDNASequence(), 50));
    ...
    protected void updateCreatureData(int ticks)
    {
        if (ticks > 0)
        {
            ...
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
                    (int) (this.getGeneticQuality() * (ticks * this.getCreature().getMaxHealth() - minHealth) / ticksToAdulthood + minHealth));
            this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
                    (float) (this.getGeneticQuality() * (ticks * (this.getCreature().getMaxStrength() - minStrength) / ticksToAdulthood + minStrength)));
            ...
            this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).setBaseValue(
                    (float) (this.getGeneticQuality() * (ticks * (this.getCreature().getMaxKnockback() - minKnockback) / ticksToAdulthood + minKnockback)));
            this.setBoundingBox((float) (this.getGeneticQuality() * (this.getCreature().getXzBoxMin() + this.getCreature().getXzBoxDelta() * (((float) this.getGrowthStage()) / 120))),
                    (float) (this.getGeneticQuality() * (this.getCreature().getYBoxMin() + this.getCreature().getYBoxDelta() * (((float) this.getGrowthStage()) / 120))));
    ...
    /**
     * Sets the scale of the this.creature depending on the age and genetic
     * quality.
     */
    private void setCreatureScale(float scale)
    {
        if (scale > 0.0F)
        {
            this.dataWatcher.updateObject(12, Float.valueOf((float) scale));
        }
        else
        {
    ...
    private void setCreatureScale()
    {
        if (this.getTotalTicksLived() < this.getCreature().getTicksToAdulthood())
        {
            ...
            this.dataWatcher.updateObject(
                    12,
                    Float.valueOf((float) (this.getGeneticQuality()
                            * (((minLength + minHeight) / 2) + (((maxHeight + maxLength) / 2) - ((minHeight + minLength) / 2))
                            * (((float) this.getTotalTicksLived()) / ((float) this.getCreature().getTicksToAdulthood()))) / ((maxHeight + maxLength) / 2))));
        }
        else
        {
    ...
    public boolean forceCreatureGrowth(EntityPlayer player, byte growthIncrease)
    {
        ...
        else
        {
            if (player != null && !player.worldObj.isRemote)
            {
                ...
                {
                    player.addChatMessage(new ChatComponentText(this.getCustomNameTag() + " (" + StatCollector.translateToLocal("entity." + this.getCreature().getCreatureName() + ".name") + ") "
                            + StatCollector.translateToLocal("entity.interaction.fullGrown")));
                }
                else
                {
                    player.addChatMessage(new ChatComponentText(StatCollector.translateToLocal("entity." + this.getCreature().getCreatureName() + ".name") + " "
                            + StatCollector.translateToLocal("entity.interaction.fullGrown")));
                }
            }
            return false;
    ...
    public void setCreatureLength()
    {
        if (this.getTotalTicksLived() <= this.getCreature().getTicksToAdulthood())
        {
            this.lengthParameter = (float) (this.getGeneticQuality() * (this.getCreature().getMinLength() + (this.getTotalTicksLived()
                    * (this.getCreature().getMaxLength() - this.getCreature().getMinLength()) / this.getCreature().getTicksToAdulthood())));
        }
        else
        {
    ...
    public void setCreatureHeight()
    {
        if (this.getTotalTicksLived() <= this.getCreature().getTicksToAdulthood())
        {
            this.heightParameter = (float) (this.getGeneticQuality() * (this.getCreature().getMinHeight() + (this.getTotalTicksLived()
                    * (this.getCreature().getMaxHeight() - this.getCreature().getMinHeight()) / this.getCreature().getTicksToAdulthood())));
        }
        else
        {
