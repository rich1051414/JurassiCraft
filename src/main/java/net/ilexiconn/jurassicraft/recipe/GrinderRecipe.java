...
public class GrinderRecipe implements IGrinderRecipe
{
    ...
    {
        if (inputStacks[0] == null && inputStacks[1] == null)
            return false;
        else if (inputStacks[0] != null && inputStacks[1] == null && input[0] != null && input[1] == null)
    ...
    private boolean isSameNBT(ItemStack target, ItemStack input)
    {
        if (!target.hasTagCompound())
            return true; //Checks whether cached recipe doesn't have #NBTTagCompound, (returns true).
        if (!input.hasTagCompound())
            return false; //If cached recipe has #NBTTagCompound and and input donsn't #NBTTagCompound, (returns false).
